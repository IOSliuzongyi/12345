...
-(void)setSelf{
    ...
    self.backgroundColor = [UIColor colorWithRed:255/255.0 green:251/255.0 blue:250/255.0 alpha:1];
}
...
//        [pic setFrame:CGRectMake(i*self.frame.size.width,0, self.frame.size.width, self.frame.size.height)];
        [pic setFrame:CGRectMake(i*self.frame.size.width,0, self.frame.size.width, 246)];
        UIImageView * tempImage = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, pic.frame.size.width, 246)];
        ...
        if ([[obj objectForKey:@"isLoc"]boolValue]) {
            ...
            NSString *requestURL = [obj objectForKey:@"pic"];
            CGRect urlImageFrame = CGRectMake(tempImage.frame.origin.x, tempImage.frame.origin.y, tempImage.frame.size.width, tempImage.frame.size.height);
            UIImage *imageDefault = nil;
            if ([obj objectForKey:@"placeholderImage"]) {
                imageDefault = [obj objectForKey:@"placeholderImage"];
            }
            URLImageView *urlImageView = [[URLImageView alloc] initWithURL:requestURL Frame:urlImageFrame Default:imageDefault param:nil];
            
            tempImage = urlImageView;
...
//                                               }];
        ...
        UILabel * title = [[UILabel alloc]initWithFrame:CGRectMake(i*self.frame.size.width, self.frame.size.height - 42, self.frame.size.width,27)];
        [title setText:[NSString stringWithFormat:@"   %@",[obj objectForKey:@"title"]]];
        [title setTextColor:[UIColor colorWithRed:165/255.0 green:50/255.0 blue:21/255.0 alpha:1]];
//        [title setFont:[UIFont fontWithName:@"Helvetica" size:20]];
    ...
    if ([self.pics count]>3) {
        scrollTimer = [NSTimer scheduledTimerWithTimeInterval:3.0f
                                                       target:self
                                                     selector:@selector(scrollTopic)
                                                     userInfo:nil
                                                      repeats:YES];
    }
}
-(void)click:(id)sender{
    [JCdelegate didClick:[self.pics objectAtIndex:[sender tag]] atIndex:[sender tag]];
}
